...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        [Tooltip("MaxPiercing of the projectile")]
        ...
        public UnityAction<GameObject> OnDamaged;
        ...
        new void OnShoot()
        {
            ...
            Collider[] ownerColliders = GetOwner().GetComponentsInChildren<Collider>();
            m_IgnoredColliders.AddRange(ownerColliders);

            // Handle case of player shooting (make projectiles not go through walls, and remember center-of-screen trajectory)
            PlayerWeaponsManager playerWeaponsManager = GetOwner().GetComponent<PlayerWeaponsManager>();
        ...
        void OnHit(Vector3 point, Vector3 normal, Collider collider)
        {
            ...
            else
            {
                ...
                if (damageable && !damaged.Contains(damageable))
                {
                    damageable.InflictDamage(Damage, false, GetOwner());

                    damaged.Add(damageable);
                }

                ShootableButton shootableButton = collider.GetComponent<ShootableButton>();
                if (shootableButton)
                {
                    shootableButton.OnShoot(GetOwner());
                }
            }
            ...
            OnDamaged?.Invoke(collider.gameObject);
