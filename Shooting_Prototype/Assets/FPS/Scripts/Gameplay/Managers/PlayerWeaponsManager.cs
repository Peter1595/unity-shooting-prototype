...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        public WeaponController GetWeaponByName(string name)
        {
            for (var index = 0; index < m_WeaponSlots.Length; index++)
            {
                var w = m_WeaponSlots[index];
                if (w != null && w.WeaponName == name)
                {
                    return w;
                }
            }
        ...
        public WeaponController PowerUpWeapon(string weaponName, float ratio)
        {
            WeaponController Weapon = GetWeaponByName(weaponName);
            ...
            Weapon.AmmoReloadDelay /= 1 + ratio;

            Weapon.DelayBetweenShots /= 1 + ratio;

            Weapon.MaxChargeDuration /= 1 + ratio;

            if (Weapon.PowerUps % 3 == 0)
            {
                //Debug.Log("power ups: " + Weapon.PowerUps);

                Weapon.AmmoReloadRate *= 2;
                Weapon.MaxAmmo *= 2;

                if (Weapon.BulletSpreadAngle <= 0)
                {
                    Weapon.BulletSpreadAngle = 1;
                }

                Weapon.BulletSpreadAngle *= 2;
                Weapon.BulletsPerShot *= 2;

                if (Weapon.BulletSpreadAngle > 15) {
                    Weapon.BulletSpreadAngle = 15;
                }
            }
